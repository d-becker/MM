...
public:
...
#ifdef MM_NO_FUSED
    compute_no_fuse(func, std::forward<ARGS>(args)...);
#else
    compute_fuse(func, std::forward<ARGS>(args)...);
#endif
	}

	template<typename FUNC, typename ...ARGS>
	void compute_no_fuse(FUNC func, ARGS ...args) {
    ...
    for (std::size_t i = 0; i < structure.cell_number(); i++) {
      ...
      // TODO: We need the coordinates for the iteration range, but it is
      // unfortunate.
      ...
      if (index_generator.is_in_range(coords) && cell.nmats == 1) {
        ...
        func(args.get(data, cell_mat_index, value_index)...);
      }
    }
  ...
  template<typename FUNC, typename ...ARGS>
  void compute_fuse(FUNC func, ARGS ...args) {
